...
> **상태: 3A단계 병합 (2026년 9월).** 1단계 — 뼈대: `collect`, `check`,
> waiver, 종료 코드, 끝까지 흐르는 컨트롤 여덟 개 — 와 2단계 — 자동 판정 가능한
> KISA 항목 전체: DISA STIG·NIST SP 800-53 참조를 갖춘 기반 작업부터 계정, PAM,
> sshd, 홈 디렉터리, 시스템 파일, 서비스, 방화벽, 로깅, NFS·SNMP·패치 위생,
> FTP·메일·DNS, 마무리 커버리지·참조 게이트까지 — 그리고 3A단계 — 심층 파일시스템
> 워크 — 가 병합되었습니다. 67개 중 67개 항목이 등재되어 있습니다(컨트롤 68개:
> auto 57, partial 7, manual 4 — manual은 근거를 함께 제시). `collect --deep`은
> 로컬 파일시스템을 시간·항목 예산 안에서 한 번 걷되 심링크를 따라가지 않고 원격
> 마운트나 컨테이너 계층에 들어가지 않으며, setuid·world-writable·소유자
> 없음·숨김 항목 하나하나를 소유 패키지와 결합합니다 — RHEL 계열은 rpm의 파일
> 표, Debian 계열은 dpkg의 파일 목록과 선언 모드 기준 목록으로 — 그래서 U-15,
> U-23, U-25, U-33이 검토 대상이 아니라 근거로 판정됩니다. 어떤 항목을 어떤
> 컨트롤이 어떤 자동화 등급으로 판정하는지, 컨트롤이 읽는 팩트 키가 무엇인지는
> [커버리지 표](docs/reference/coverage.md)로 생성되어 CI가 검사합니다. CI는 또한
> 워크를 포함한 수집기를 VM에서 root로, 일반 사용자로, Ubuntu 22.04/24.04·Rocky/Alma
> 9 컨테이너 안에서(Debian 12는 실패해도 CI를 막지 않는 카나리로 함께), 네트워크
> 없는 읽기 전용 컨테이너에서 돌리고, root 없이는 `denied`·systemd 없이는
> `unsupported`·컨테이너 안에서는 워크가 `unsupported`여야 하는 팩트의
> [capability matrix](docs/reference/capability-matrix.json)에 대조합니다.
> 아키텍처, 계약, 릴리스 범위는
> [설계 스펙](docs/superpowers/specs/2026-09-02-muster-design.ko.md)에 정리되어
> 있고, 판정을 바꾸는 변경은 [CHANGELOG.md](CHANGELOG.md)에 기록합니다. 아래의
> 남은 3·4단계 항목은 계획이지 약속이 아닙니다.
...
글로벌 벤치마크는 두 번째 규칙집이 아니라 교차 참조입니다. 68개 컨트롤 중 25개가
...
2. **두 배포판, 자동 판정 가능한 KISA 항목 전체.** Ubuntu와 Rocky 수집기. 67개 중 64개를
   자동 판정하거나 자동 근거를 붙이고, 파일시스템 순회가 필요한 3개 — 파일·디렉터리
   소유자(U-15), SUID/SGID/sticky 파일(U-23), 숨겨진 파일(U-33) — 는 워크로 유예.
   매핑이 있는 모든 컨트롤에 DISA STIG와 NIST SP 800-53 참조 번호를 추가하고,
   `controls lint`가 모든 KISA 참조를 67개 항목 인벤토리와 교차 검사.
3. **심층 파일시스템 워크(3A, 병합됨), 그 다음 목록 밖의 고가치 점검과 프로파일**,
   같은 수집기에서. 3A: `collect --deep`이 유예된 3개 항목을 등재하고 U-25가 실제
   호스트를 판정하게 함 — 67개 중 67개, 컨트롤 68개(auto 57, partial 7, manual 4 —
   manual은 근거를 함께 제시). 유지보수자 도구 `tools/suidindex`가 공개 컨테이너
   이미지로부터 릴리스별 선언 모드 기준 목록을 생성. 다음: 패키지 검증(`rpm -V` /
   `dpkg --verify`), 파일 capability와 ACL, cron·타이머 인벤토리,
