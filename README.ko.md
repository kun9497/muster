...
> auto 57, partial 7, manual 4 — manual은 근거를 함께 제시),
> 그리고 가이드 밖 19개가 더 있습니다. `collect --deep`은
...
- **종료 코드**는 계약입니다. `2`(실행 불가 또는 신뢰 불가) > `1`(findings 있음)
  ...
  가이드 밖의 FAIL도 다른 FAIL과 같이 셉니다: 67항목에는 깨끗하지만 커널·부트·마운트
  설정이 스톡인 호스트는 `1`을 읽으며, 요약의 두 범위가 어느 쪽인지 말해 줍니다.
...
3. **심층 파일시스템 워크(3A, 병합됨), 그 다음 목록 밖의 고가치 점검과 프로파일**,
   ...
   가이드 밖으로 나뉨. 가이드 밖 FAIL도 프로파일이 오기 전까지는 종료 코드에 셈. 다음:
   패키지 검증(`rpm -V` / `dpkg --verify`), 파일 capability와 ACL, cron·타이머
   인벤토리, `authorized_keys` 인벤토리, 삭제된 바이너리로 도는 프로세스, 감사
   파이프라인 상태, 패치 위생, 리스닝 소켓과 방화벽 규칙의 노출면 교차 판정. 프로파일이 컨트롤과 파라미터를 고릅니다. `kisa-unix-2026`이 기본이고,
