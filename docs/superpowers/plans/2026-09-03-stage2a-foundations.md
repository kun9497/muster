...
- **Post-review CI fix (R124).** The first pull-request run failed the non-root and every container leg on the invariant "a PASS carries only `ok` evidence": `hosts_lpd_permissions` is the first control with `absent_means: pass`, and by spec §6.5 its PASS carries the `absent` envelopes as evidence (`reason: absent counts as pass: …`). The invariant was written before any such control existed; the two `jq` assertions now allow `absent` evidence only on a result whose reason starts with `absent counts as pass`, and still refuse `missing`, `denied`, `error`, `timeout` and `unsupported` under any PASS. Verified against a real snapshot on the lab host (true on the real report, false once its `absent` statuses are rewritten as `denied`).
