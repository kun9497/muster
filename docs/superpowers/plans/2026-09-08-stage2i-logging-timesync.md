...
## Execution notes (2026-09-08, SDD, controller = Fable 5.1)

Executed via subagent-driven development in worktree `stage2i-logging-timesync` over main @ `d2bac56` (2H merged). Delivered: a shared three-tier drop-in merge helper (`dropin.go`, `mergeDropins` — earlier dir shadows the same basename in later dirs, survivors in basename order, later wins); the `ntp`/`syslog` rows of the table-driven services collector (units chrony/chronyd/systemd-timesyncd/ntpd/ntp/ntpsec and rsyslog/syslog-ng, no ports); the `timesync` collector (`timesync.go` — provider chrony/timesyncd/ntpd/none from persisted config, servers counted from the files including chrony `refclock`/`sourcedir`/`confdir` and timesyncd `NTP=`/`FallbackNTP=`/`LinkNTPServers=`/`RuntimeNTPServers=`, two separate `timedatectl` oracles for `synchronized` and the timesyncd effective server list); the `logging` collector (`logging.go` — implementation rsyslog/syslog-ng/journald-only/none, the v1 rsyslog model parsed in rule order into a per-facility `coverage` list of the fixed 20 facilities, `log_targets` with a modification-time age, and the journald `storage`/`forward_to_syslog`/`persistent` leaves derived through `mergeDropins`); `collect.ReadMeta.ModTime` with the snapshot's `CollectedAt` as the only clock (I-27); 22 new fact keys (additions-only, `schema_version` 1, first use of `subject_kind: facility` on a `list<record>`); the new `log` category; and two controls **U-65** `muster.log.time_sync` and **U-66** `muster.log.syslog_policy`. **Control set 45→47 (auto 43, partial 4, manual 0).**

**The load-bearing decisions.** (1) **KISA's any-provider criterion, not the STIG's chrony-only one:** U-65 passes on chrony, systemd-timesyncd or ntpd (active unit + at least one configured source); `synchronized` is evidence only, so a freshly booted or air-gapped host is not a finding; only the rhel9 rules are cited and the Ubuntu chrony-only/timesyncd-forbidden rules deliberately are not (opposite polarity). (2) **`timedatectl` failure is always `unsupported`** (I-3, the R220 cronTimers shape, not the firewall euid split) — the command is unprivileged, so its failure is an environment limitation; the timesyncd server list is then `absent` and U-65 reads MANUAL rather than guessing. (3) **The rsyslog model under-claims (H-16):** classic selectors, omfile/omfwd, `@`/`@@` forwarding, discard and includes are modelled; anything else counts in `unmodelled` and makes `coverage` ABSENT → MANUAL with the parse evidence; property filters are coverage-neutral (I-12); a discard ahead of a covering rule removes the facility (I-13); `.none` is scoped to its selector line (I-14); `persistent` means a local file target, not that the file exists (I-15); syslog-ng is not parsed (MANUAL); a host with `implementation: none` falls through "no mechanism applies" to MANUAL because sysklogd/BusyBox/metalog exist and v1 does not detect them (I-11 — the plan's third mechanism could never have produced FAIL and was dropped). (4) **No clock in a collector** (I-27, surfaced by the 2J pre-flight): `mtime_age_s` is `CollectedAt − ModTime`, omitted for an unseeded stat, so the same snapshot always yields the same bytes. (5) **journald two-sided `storage`** (persisted `Storage=` merged through the drop-in chain, runtime by the `/var/log/journal` directory) with `persistent` judged from both (I-8); an unreadable chain degrades all three leaves together (C3).

**Rulings.** Controller boundary rulings I-1…I-8; pre-flight rulings **I-9–I-26** (folded pre-execution — the Fable pre-flight decoded the control blocks through the real loader and caught 3 BLOCKING plan defects: four decode errors in the U-66 block, `log_targets` stats of undeclared paths that would have violated the read guard, and a mechanism 3 that could never FAIL); **I-27** cross-plan (the 2J pre-flight found that no `ReadMeta` modification time existed — added here, reused by 2J after rebase); whole-branch rulings ****IR-1…IR-19** (one consolidated fix wave, commits `b604eb1` timesync and `fe6c736` logging)**. All five tasks passed task review; Task 3 took one test-only fix round (a denied stat of `/var/log/journal` itself had no test — added, mutation-proven). Task 4's two YAMLs decoded and linted on the first attempt with no deviation from the plan; Task 5 reconciled 45→47 with `staticcheck` clean.

**Whole-branch review (ultracode workflow: five Fable lens finders + adversarial refuters): five Fable lens finders (key emission and degradation, read guard and determinism, the rsyslog model, controls and tests, cross-task seams) produced 22 findings — BLOCKING 1, MEDIUM 11, LOW 10 — and 63 confirmed-correct items; adversarial refuters verified 8 and the controller adjudicated the rest from the cited code. The fix wave resolved: the BLOCKING rsyslog failover recipe (`$ActionExecOnlyWhenPreviousIsSuspended` and the interval/nth-time directives and their RainerScript parameters made a conditional local buffer count as persistent logging → now `unmodelled` → MANUAL, IR-11); five more silently-skipped constructs that can discard or reroute a required facility — a bare `stop`/`~` (IR-10), `$RuleSet`/`$DefaultRuleset` (IR-12), `action(type="omdiscard")` (IR-13), property filters on the facility/severity/priority properties (IR-14; `msg`/`syslogtag`/`programname` filters stay neutral per I-12), priority-restricted discards such as `kern.=debug ~` (IR-15, which had produced a confident-wrong FAIL) and numeric selector spellings (IR-17); a `/dev/console` target counted as a persistent file (IR-2, now a `device` kind); a backslash-continued Debian selector split into fabricated user-target rules (IR-4); the `^program` action kind (IR-16); chrony `include` and ntpd `includefile` sources dropped so an active chrony with its pools in an included file read as a confident-wrong FAIL (IR-1 — declared includes are followed, undeclared ones make the server list absent → MANUAL); the provider order chrony → ntpd → timesyncd so an ntp/ntpsec host with the leftover timesyncd conffile is judged from its own file (IR-5); and — the seam finding that mattered most — a logger is detected by its binary, not by a conffile: Debian keeps `/etc/rsyslog.conf` after `apt remove rsyslog` (the Debian 12 journald migration) and after `apt install syslog-ng`, and both hosts had been judged by the dead rsyslog rules — a confident-wrong FAIL with journald persistent, a confident-wrong PASS on a forwarding-only syslog-ng (IR-7; `/usr/sbin/rsyslogd`, `/usr/sbin/syslog-ng` and their `/usr/local/sbin` twins are stat-declared; U-66's description now states that an installed daemon is the logger judged, that a stopped daemon is a finding even with a persistent journal, and that a leftover conffile does not count). Tests pin all six rsyslog leaves on the absent path (IR-18); `log_targets.stat_status` names the real status (IR-9); the `facilities` parameter's description lists the 20 names it can match (IR-19). Scoped re-review: PASS (Fable; BLOCKING 0, MEDIUM 0, LOW 5 — every ruling is pinned by a structural test that reads red without its code path, both deviations verified, stock Ubuntu/Debian/RHEL constructs walked line by line and kept at `unmodelled` 0). The five LOW residuals are parked below.**

**Lab host:** an Ubuntu 22.04 chrony + rsyslog node — root collect complete; `time_sync` provider chrony with 3 sources and `synchronized` true; `logging` implementation rsyslog, `parse_complete` true, `unmodelled` 0, `property_filters` 2, `coverage` 20 records with every required facility logged persistently; journald persisted `auto` / runtime `persistent`; **U-65 PASS, U-66 PASS**, 47 controls loaded. (`collect --out` under `/tmp` is refused as world-writable — use `/var/lib/muster`.)

**Process:** the user's parallelism question was answered mid-plan with an evidence-backed verdict (single lab wipe target, cwd-pinned sessions, registry EOF merge collisions, controller-serial bottleneck) → "stagger-lite": the next plan's scope/plan/pre-flight (2J) was authored during this plan's SDD waits, its SDD starts only after this merge, and the lab token belongs to one plan at a time.

**Parked:** `logging.logrotate.*` (I-7); migrating the pwquality/faillock inline drop-in copies onto `mergeDropins` (2M); `rsyslog.rules` row volume on real hosts (evidence only — 2M if snapshot size matters); `$IncludeConfig` with a trailing-slash directory expands outside the declared `*.conf` glob and reads MANUAL (no distro ships that form); a log modified after `collected_at` gets no age (I-27 forbids fabrication); the raw-vs-cleaned rsyslog include glob (IR-3, refuted — both resolve identically through `filepath.Glob`); `filepath.Glob` swallowing EACCES on a declared directory so a non-root run lists nothing instead of `denied` (IR-8 — a shared `hostAccess.Glob` primitive change → 2M); a leftover conffile of a removed time provider still deciding `time_sync.provider` (IR-6, by design — the registry description says so; purge the configuration of a removed provider); engine-side validation when an `each … where` selects no record (a `facilities` override outside the 20 names passes vacuously — IR-19 documents it; the evaluator change → 2M); the re-review's LOW residuals — the `time_sync.servers` description does not list the include/depth absent causes, the include depth cap has no test, a self-including chrony file cites the main file twice, the leftover-conffile reason does not name the two probed binary paths, and a program/pipe-only facility carries an empty coverage reason (all evidence wording; no verdict changes).
