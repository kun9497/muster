...
// internal/check
type clauseOutcome struct { ...; Reason string; Vacuous string }               // Reason: clause-level reason (M-5); Vacuous: param name (M-6)
...
// -check fails when README.md lacks "64 of the 67 items" or README.ko.md lacks "67개 중 64개" (temp READMEs via a -readme-dir flag or a package-level var).
...
```
`main.go`: load the inventory with `controls.LoadKISA(dir)` (flag `-kisa-dir docs/reference/kisa` replaces `-kisa`), compute `controls.FactUsage`, and in `-check` mode also verify the README fragments (flag `-readme-dir .`); the failure message names the fragment and the file. Regenerate `docs/reference/coverage.md`; edit the two README numbers.
...
// fixtures_test: a MANUAL result whose evidence carries a status "missing" entry fails the suite (drive it with a temp fixture before wiring the four controls).
func TestManualFixturesCarryEveryEvidenceLeaf(t *testing.T) {}
```
- [ ] **Step 2: Run red.**
- [ ] **Step 3: Implement.** `value.go`: `paramName`. `collection.go`: `fieldClause` returns `missingFieldError{Field}` for a nil value under any op but present/absent; `evalCollection` — on `errors.As(err, &mf)` from `where` or `require`: append `Observation{Subject: subject, Expected: <sub.Expected substituted>, Actual: nil, Verdict: "fail"}`, set `out.Holds = false`, `out.Reason = fmt.Sprintf("element %s has no field %q", subject, mf.Field)` (first one wins), continue the loop; count `selected` for `each`; after the loop, when `cl.Op == "each" && cl.Where != nil && len(list) > 0 && selected == 0`: append the `<kind>:*` observation and, if `paramName(cl.Where.Expected)` is a reference, set `out.Vacuous = name`; build the evidence value string per M-7 (subjects in observation order, capped at five). `eval.go` steps 11-14: after `evalClause`, `if out.Vacuous != "" { res := fail(r, MANUAL, "", fmt.Sprintf("parameter %q selected no element of %s (%d elements)", out.Vacuous, cl.Fact, n)); res.Evidence = append(r.Evidence, out.Evidence...); return res }` (n from the evidence); when `!out.Holds && r.Reason == ""`: prefer `"clause does not hold: " + out.Reason` when set. Step 4: `if len(c.Evidence) > 0 { r.Evidence = dedupeEvidence(append(r.Evidence, e.evidenceFor(c.Evidence)...)) }`. The four control YAMLs gain `evidence:` per M-8; their `manual-*.json` gain the leaves (`ok` values consistent with the file's story). `fixtures_test.go`: after evaluating a `manual-` fixture assert no evidence status is `missing`.
- [ ] **Step 4: Gates.** `go test ./internal/... ./cmd/muster/ -count=1` (the report goldens regenerated and diffed by eye); lint `ok: 64 controls`; coverage `-check` (unchanged — no automation change).
...
// cron (already routes): a denied spool dir → cron.files/cron.dirs denied — re-enable the R204 case 2F removed, now that the double can express it.
func TestCronDeniedSpoolIsDenied(t *testing.T) {}
```
- [ ] **Step 2: Run red** (cross-compile on Windows; the Linux run on the lab host).
- [ ] **Step 3: Implement.** `registry.go`: `globDir(pattern) string` (literal prefix per M-10) and the probe in `hostAccess.Glob`; `Getxattr` per M-11. `run.go`: after the collector loop, `hdr.Redaction.RedactedFields = redactedFields(b, reg)` — every `facts.Entry` with `Sensitivity == "secret"` whose leaf (`b.leaf(key)`) is an `Envelope` with `StatusOK`, sorted. Audit the 25 call sites into the report as a table `file:line | today | after | leaves affected`; change the four sites named in M-10.
...
// an interactive account with home /srv/app → files.home_dirs absent with reason naming "app (/srv/app)"; two such accounts → both named, sorted by user; no such account → today's rows.
func TestHomeOutsideTheDeclarationMakesTheLeafAbsent(t *testing.T) {}
// /home/bob/.rhosts EACCES → files.user_rhosts denied, reason "/home/bob/.rhosts: …"; ENOENT → no row, leaf ok.
func TestUnreadableRhostsIsDenied(t *testing.T) {}
// pwquality: conf + conf.d/{10-a,20-b}.conf → last file wins per key; a denied 10-a.conf poisons every derived value (R148); an absent conf.d dir is fine; inputs list the files read in order.
...
```
- [ ] **Step 2: Run red.**
- [ ] **Step 3: Implement.** `sshd.go:378-384`: replace `ErrorEnv` with `collect.Absent(...)` + the reason. `files_home.go:45-52`: collect undeclared accounts; after the loop, if any: return `collect.Absent("home path outside the collector's declaration: " + strings.Join(names, ", "))` (source unchanged). `userRhosts` `scan`: on a read error other than ENOENT set a package-level `denied` envelope (first wins, path-prefixed via `readReason`) and return it instead of the list. `pam_derive.go:160-180`: the `readKV`+`Glob` block becomes one `mergeDropins` call; keep `apply(kv)` semantics (values map → the same keys).
...
**Files:** modify `controls/file/ip_port_restriction.yaml` + its six fixtures (+ `fail-tcpwrappers-no-libwrap.json`), `cmd/muster/testdata/full-pass.json`, `full-fail.json` (add `files.libwrap_present` ok:true; update `run.controls_version`), `cmd/muster/e2e_test.go` (M-17), `controls/VERSION`, `.github/workflows/ci.yml`; create `docs/reference/capability-matrix.json`; regenerate the report goldens (`controls_version`).
...
- [ ] **Step 1: Failing tests**
```go
// U-28 fixtures: deny-all + libwrap true + backend none → PASS (mechanism 1); deny-all + libwrap false + backend none → FAIL; deny-all + libwrap false + backend nftables restricting → PASS (mechanism 2); every fixture carries files.libwrap_present.
// e2e: assertStatuses with a want map missing one loaded id fails; with an extra id fails (drive with a copy of the map in a t.Run that expects failure via a recording *testing.T substitute or by asserting the helper's returned error — refactor assertStatuses to return error and keep a t.Helper wrapper).
...
```
- [ ] **Step 2: Run red.**
- [ ] **Step 3: Implement.** YAML per M-9; fixtures; `assertStatuses` per M-17 (loaded ids via `controls.LoadDefault`); `capability-matrix.json` per M-18 with the non-root list derived from the lab host's `runuser -u nobody` snapshot (record the derivation in the report; at least five keys, three collectors, only keys `denied` on both families by permission — never a key whose status depends on an installed package); `ci.yml`: in `collect-nonroot` after the report step and in `collect-containers` under `if [ "${{ matrix.init }}" = "false" ]`, one step:
```sh
...
```
(the container variant reads `$m[0]["no-systemd"].unsupported` and `"unsupported"`); `controls/VERSION` → `kisa-unix-2026+2026.09.09`; regenerate goldens and review that only `controls_version` (and Task 3's evidence strings, already committed) differ.
