...
Executed 2026-09-08 by subagent-driven development on branch `stage2f-system-cron` (eleven commits over `778c302`, these notes included), developed on Windows and tested on a Linux lab host as root (every Linux collector test, the real cron/spool, `/etc/systemd/system`, sudoers and `/var/log` walks, and the collect/check proof ran there; the Rocky/Alma differences — `/etc/init.d` a symlink, `/var/spool/at`, `/var/log` group `root` vs `syslog` — are exercised by synthetic fixtures and CI's init containers). Two fable reviews ran: a fresh pre-flight scan of the plan found 3 blocking and 9 should-rule defects; the whole-branch review after execution found 0 blocking and 5 should-fix. One consolidated fix wave cleared the whole-branch findings. Ruling numbers continue the ledger (R1–R201 belong to plans 1A–2E; R202–R213 are pre-flight, R214–R219 are the final fix wave).

- **The cron collector (T1).** `cron.files`/`cron.dirs`/`cron.timers`. **R203:** the file enumeration skips a non-regular match (a spool glob matches the `crontabs` directory) and uses the real at-spool paths (`/var/spool/cron/atjobs`, `/var/spool/at`). **R202:** a denied stat is a `reason`-bearing row (a finding), a symlink is `is_symlink:true` with no reason, ENOENT is no row. **R212:** a user-spool file's `owner_ok` is `uid == 0 || uid >= 1000` (no `/etc/passwd` read). **R213:** the timer command (`systemctl list-unit-files --type=timer`) runs as any user, so its `commandFailure` is `needsRoot=false` despite the collector's `Needs: root` (which only annotates `--list-actions`; `run.go` does not gate on it).
- **files startup/syslog/inetd (T2).** `permRow` fills every field before branching; a directory match is skipped (`/etc/systemd/system/*` matches `.wants`/`.d` dirs), an `ErrSymlink` match is an `is_symlink` row, a denied stat carries `reason`. **L5:** the inetd/xinetd path constants are reused from `services.go` (only `xinetdConfPath` is new).
- **files sudoers/log (T3).** **R210:** `acl_present` uses `aclEntries` (denied on a non-root run). **R211:** the `/etc/sudoers` content read uses a 256 KiB `sudoersReadLimit`. **R206:** `logTree` includes `/var/log` itself. **R215:** `logTree` propagates `groupNames`' read error/truncation (the R70 convention) so a denied `/etc/group` does not become a silent `group:""` that U-67 would then FAIL on fabricated evidence; the cron/startup rows carry `group` as evidence only. A denied `/etc/sudoers` content read is the path-prefixed error on `sudo.includedir`/`secure_path` (C3) while `sudo.installed` stays true; the shared `fsAccess.Stat` test double now consults its `stats` map before `fails` so a 0440 file is stat-able but read-denied (only `/etc/sudoers` is in both maps, so no other test changed).
- **The mixed-absence and vacuous-each hazards.** A failed glob is the read error, never `[]`; every control that judges a list carries `none where {reason present}` so a denied row is a finding; a genuinely empty enumeration is an ok `[]` (a vacuous `each` correctly passes). U-20 uses `mechanisms` (inetd vs xinetd) with `absent_means: pass` so a stock host with neither super-server passes. **R205:** every fixture carries the leaves its control references, as `absent` where not the focus, so none ERRORs on a missing key. **R204:** `filepath.Glob` swallows a denied *directory* (returns `[]`, nil) — an accepted pre-existing project-wide limitation (root collect, muster's primary mode, reads these dirs); the three misleading denied-glob tests were removed and a guarded `ReadDir` primitive is carried forward.
- **Controls (T4/T5).** U-17 (startup), U-20 (inetd), U-21 (syslog), U-37 (cron), U-63 (sudoers), U-67 (log dir). **R216:** U-17 and U-20 judge root ownership (`uid == 0`) their descriptions had promised. **U-67 (D09):** the log-group allowlist is the control's `params.allowed_groups`, judged by `each where {group_writable} require {group in allowed}` — not precomputed in the collector. U-21 and U-37 FAIL a stock host by the guide's 0640 criterion (both families ship 0644), stated in their descriptions; the e2e snapshots use 0640 so the suite is green.
- **The run-parts bug (fix wave, R217).** `/etc/cron.{hourly,daily,weekly,monthly}/*` are executable run-parts scripts (0755); the mode-≤0640 clause FAILed every one forever and the "chmod 0640" remediation would have broken cron. The collector now tags them `scope: "periodic"`; U-37's mode-subset clause is scoped to `system`/`access`/`user` and periodic scripts are judged on ownership and writability instead; the remediation limits `chmod 0640` to crontab-format files, and the misleading `RHEL-09-232040` reference ("do not modify cron perms from OS defaults") was dropped. Also in the fix wave: the two missing reason-guards (R214), the `/etc/group` honesty fix (R215), the root-ownership clauses (R216), the R203 directory-skip unit test (R218), and the RHEL `/etc/rc.d/rc[0-6].d` globs, the synthetic-sudoers-fixture self-label, and a spaced `secure_path =` fixture.
- **Real-host proof (root, stock Ubuntu 22.04).** `collect --require-root --require-complete` exit 0; verdicts `startup_script_permissions PASS`, `inetd_conf_permissions PASS` (no super-server), `syslog_conf_permissions FAIL` (stock 0644), `cron_permissions FAIL` (on `/etc/crontab` and `/etc/cron.d/*` at 0644, **not** on the 0755 periodic scripts — the R217 fix), `sudoers_permissions PASS`, `log_dir_permissions PASS`; `cron.files` 14 rows, `log_dirs` 40, `sudo.secure_path` read. No host identifier, address, alias or credential is in any tracked file (every commit greps clean).
- **Carried forward.** R219 (accepted, safe-direction): a crontab-format config file that is itself a symlink (`mode -1`) is now mode-judged and would FAIL — a rare, non-real configuration that over-reports (never a false PASS, and a denied read is still caught by the reason-guard); a compound-`where` or a precomputed mode-checkable selector is the deferred fix. R204: a guarded `ReadDir` primitive so a denied enumeration directory is ERROR rather than an empty list (a dedicated hardening / 2M). N5: U-37's `absent_means: fail` is unreachable from the collector (it always emits `cron.files`). The registry added 28 keys (the plan text's "20" was an undercount).
