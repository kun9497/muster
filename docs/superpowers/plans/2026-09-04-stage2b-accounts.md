...
Executed 2026-09-04 to 2026-09-07 by subagent-driven development on branch `stage2b-accounts` (13 commits over `72174f0`, these notes included), developed on Windows and tested on a Linux lab host as root (every Linux test, the real-host run of the ten controls and the final `-race -shuffle` run happened there; the container legs are validated by CI). A pre-flight scan found 1 blocking, 9 should-rule and 17 lesser defects in the task text; every derived value the plan stated (13 passwd rows, 2 failures, 11 groups, the administrative-member order, the warn minimum, `carol`'s `legacy` hash, UMASK on line 10) was re-computed and confirmed before execution. Where the task text disagreed with the spec or the code it was amended, and the spec won (ruling numbers refer to the execution ledger; R1–R124 belong to plans 1A, 1B and 2A):

- **Lint without an index (T6, R125).** `internal/controls/lint.go` reported every STIG reference as "cannot be verified" when no index was loaded, contradicting R120 and the documentation; it now checks the shape only (`ValidSTIGID`, `^[A-Z0-9]+-[0-9]+-[0-9]+$`) and verifies existence only with `--references`, which `make lint-controls` and CI pass. Plain `controls lint` on the embedded set stays exit 0.
- **`/etc/shells` (T2, T6, T7, R126).** `accounts.shells` is never absent: a missing file yields the libc default (`/bin/sh`, `/bin/csh`) with a reason, an unreadable one carries the read's status; `shell_valid` exists on every row, and the two shell controls screen on `accounts.shells` first so a denied read is `ERROR`, never a judgment on a guess.
- **NSS (T4, R127, R128, R137).** `sss` counts as a remote source only when `/etc/sssd/sssd.conf` exists (authselect lists `sss` on every RHEL-family host); any other non-local source counts unconditionally; the step-13 hook fires only for clauses on facts whose registry subject is `user` or `group`, or on `accounts.shadow_in_use`, so `login_defs.*` controls are not degraded; nsswitch lines are cut at `#` and bracketed action blocks are skipped by depth, as glibc does.
- **Observations (T4, R129).** `${param}` is substituted in observation text (the R27 rule applied to `each`/`none`).
- **The user record (T2, T6, R130–R132, R138).** `shadow_passwords` names unshadowed accounts through a `shadowed eq false` clause; truncation flags cross every join (passwd|shadow, passwd|group); a passwd row without a shadow row carries `password_status: noshadow` and empty hash fields; a missing `/etc/shadow` is a known state — every row is `noshadow` — while a denied one leaves the rows without shadow fields and the control screens on `shadow_in_use`. A locked account keeps its hash algorithm (`!$1$…` → `$1$`); a hash without a `$id$` prefix is `legacy`.
- **Partial controls and sensitivity (T3, T7, R133, R134).** `unnecessary_accounts` and `admin_group_minimal` use `absent_means: manual`; the name-carrying lists (`groups`, `orphan_gids`, `admin_group_members`) are `sensitivity: internal`.
- **Tests carried between tasks (R135, R136).** Task 3 pinned the passwd-unreadable branch, passwd truncation and the R126 reason; Task 4 pinned a non-numeric gid and the derived-source inputs.
- **Not built (plan §File structure).** `accounts.last_login`, `accounts.duplicate_uids`, `accounts.nis_domain`; the shadow `rounds` value spec §5.6 lists (deferred with a note for 2M); the `references.kisa` cross-check (2M).
- **Guide-literal defaults that fail stock hosts (§6.6).** `shadow_permissions` allows 0400 or stricter, so Ubuntu's root:shadow 0640 fails until `allowed_modes` is widened; `unnecessary_accounts` names `lp`, `uucp`, `nuucp`, so stock Ubuntu and Rocky report `WARN`; both are stated in the controls' descriptions.
- **Final review (fable) and the fix wave (R139–R141).** Merge-ready with no blocking finding; one follow-up commit widened `password_hash_algorithm`'s defaults to include scrypt and bcrypt (`$7$`, `$2b$`, `$2y$`, `$2a$`; `BCRYPT`) because the guide's intent is "SHA-256 or stronger" (R139), made `accounts.shadow_in_use` `internal` (its reason names an account), corrected two control descriptions (`admin_group_minimal` lists the wheel/sudo/admin members in the snapshot rather than "as evidence"; `system_account_shells` says `nobody` and accounts at or above UID_MIN are not judged and that "login shell" means listed in `/etc/shells`), pinned the four unpinned branches (the `shadow_in_use` clause of the step-13 hook, the subject-kind-driven degradation of `primary_group_exists`, `intOr` on an unreadable login.defs, `shadow_in_use`'s truncation), exempted `accounts.nss.remote` from the unused-keys note (the engine reads it, like `sshd.collect_method`), and recorded in `THREAT_MODEL.md` that a non-root run cannot read `/etc/shadow`'s ACL. Two rulings without code: a denied or errored `accounts.nss.remote` does not degrade account controls — unknown is not "has a remote source" (R140); the `files` collector keeps `Needs: none` although the ACL xattr read makes a non-root run partial on `/etc/shadow` (R141). Real-host proof on stock Ubuntu 22.04 (root): collect complete, check exit 1; `unnecessary_accounts` WARN (`lp`, `uucp`), `shadow_permissions` FAIL (root:shadow 0640), the other eight account controls PASS; `accounts.nss.remote` false with both inputs cited; no `noshadow` row. Non-root: the run is partial (`accounts`, `files`, `sshd`), the shadow-dependent controls are ERROR, every PASS carries only `ok` evidence.
- **Carried forward.** A record lacking a named field still yields `ERROR(internal_error)` where spec §5.7 says "fails that clause with a reason naming the field" (2M, with the ACL-observation and cross-check items from 2A); NIS `compat` `+` lines count as parse failures and unshadowed rows; any future control that reads `shell_valid` must screen on `accounts.shells` (a lint rule candidate for 2M); the shadow `rounds` value of §5.6; pure-ftpd's `ftpuser` name for U-55 (2L).
