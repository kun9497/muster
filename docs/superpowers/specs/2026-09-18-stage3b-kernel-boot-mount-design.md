...
| | `boot.grub_password_set` | `bool` | true iff an uncommented line of grub.cfg, `/boot/grub2/user.cfg` (0600 on EL) or a regular `/etc/grub.d/*` carries a literal `grub.pbkdf2.` hash — `password_pbkdf2 <user> grub.pbkdf2.…` or EL's `GRUB2_PASSWORD=grub.pbkdf2.…`; `set superusers` alone proves nothing (EL's `01_users` template emits it into every grub.cfg) and `${GRUB2_PASSWORD}` is a reference, not a hash; an unreadable file among them is the read's status (C3) |
...
| `modules` | `kernel.modules` (`subject_kind: module`) | `list<record>` | one row per candidate `cramfs`, `freevxfs`, `jffs2`, `hfs`, `hfsplus`, `udf`, `usb-storage`, `dccp`, `sctp`, `rds`, `tipc`: `name`, `loaded` (`/proc/modules`), `builtin` (`/usr/lib/modules/<release>/modules.builtin`; `/lib/modules` only when `/lib` is a real directory — on merged-`/usr` hosts it is a symlink, and a container image with no `/usr/lib/modules` reports the tree `unsupported` rather than tripping over the link), `available` (`modules.dep`), `blacklisted` (a `blacklist <name>` line), `install_disabled` (`install <name> /bin/false` or `/bin/true`), `disabled` (derived: `!loaded && !builtin && (install_disabled || !available)`), `sources` (`list<string>`: the modprobe.d files that mention the module, plus the one sentinel `built into the kernel` when `builtin`); `modules.dep` names are matched with their `.ko`, `.ko.zst`, `.ko.xz` and `.ko.gz` suffixes stripped; modprobe.d is `/etc/modprobe.d`, `/run/modprobe.d`, `/usr/local/lib/modprobe.d`, `/usr/lib/modprobe.d`, `/lib/modprobe.d` in that precedence (kmod's order), `.conf` files only, masking by name as modprobe.d(5) says; a module name matches with `-` and `_` folded |
...
| 9 | `bootloader_password` | 중 | `boot.grub_password_set eq true` (a literal `grub.pbkdf2.` hash, never the template's `set superusers`); `absent_means: not_applicable`; a `denied` read (EL's 0600 grub.cfg without root) is ERROR, as every denied fact is |
...
`denied` statuses, which is what proves the EL path — after laying out the shape the images
do not ship (`/boot/grub2` 0700 with a 0600 grub.cfg inside), the way the root job writes
the sshd drop-in.
