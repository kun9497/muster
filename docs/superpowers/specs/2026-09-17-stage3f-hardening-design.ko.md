...
도는 뮤테이션 테스트(F-1…F-4), 커밋된 코퍼스를 갖고 밤마다 도는 파서 진입점당 퍼즈 타깃 하나(타깃 30개: 수집기 26, `pkgfiles` 4)(F-5…F-7),
...
5737 주소)이 됩니다. `machine_id_hash`와 `boot_id`는 해시라 그대로 둡니다. 리포트는 고쳐 쓴 파일에서
