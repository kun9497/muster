...
In Korean security practice this kind of check is a CCE assessment (configuration vulnerabilities, as opposed to CVE). muster is the CCE checker for the Linux server asset class: KISA is the primary standard; global benchmarks — CIS Benchmarks, DISA STIG, NIST SP 800-53 — are cross-references on every control and, from stage 3, selectable profiles over the same facts (section 10.2). The guide's other asset classes (Windows, DBMS, web/WAS, network and security devices, PCs, cloud) would be sibling tools sharing these contracts, never this codebase (section 10.4).
...
**Global references.** `references.cis` (benchmark name, version, recommendation number) is present from stage 1: CIS Ubuntu Linux 22.04 LTS v2.0.0, CIS Ubuntu Linux 24.04 LTS v1.0.0, and CIS AlmaLinux OS 9 v2.0.0 (used for Rocky Linux 9 as well). Stage 2 adds `references.stig` and `references.nist_800_53` wherever a mapping exists, from DISA's public STIGs — Canonical Ubuntu 22.04 LTS STIG V2R9, Canonical Ubuntu 24.04 LTS STIG V1R6, and the Red Hat Enterprise Linux 9 STIG V2R9 applied to Rocky and AlmaLinux 9 as a documented inference — with the NIST SP 800-53 control ids derived from each STIG rule's CCI through DISA's CCI list. A committed generator builds the index under `docs/reference/stig/` (rule id, group id, severity, title, CCIs, NIST controls) from sources pinned by URL and digest; lint accepts only ids present in the index; adding a key under `references` does not change the control schema version. References are identifiers and titles only — no CIS Benchmark or STIG check text — and muster judges its own controls, never a benchmark's official compliance level (D24).
...
`schema_version` is an integer. Adding a key does not bump it (the key's `since` records when it appeared); changing a key's type or meaning, or removing one, does. Adding a field to an existing `record` or `list<record>` element is an addition too and does not bump it; a clause that names a field an older snapshot lacks fails that clause with a reason naming the field, never `PASS`. `check` refuses a snapshot with a higher version (`exit 2`, `schema_mismatch`) and reads a lower one; a registered key the snapshot does not carry reads as `missing` (section 5.2), and every control that references it is `ERROR(missing_fact)` — never `PASS`, and never resolved by `absent_means`. A control declares `requires_facts: ">=N"` for an integer N (the only form accepted); a control whose requirement the snapshot does not meet is `ERROR(missing_fact)` naming the version, without evaluation. A reflection-generated schema golden file and a corpus of old snapshots under `testdata/snapshots/v<N>/` keep the rule honest (D17).
...
**Operators.** Fourteen scalar operators — `eq ne in not_in lt lte gt gte matches not_matches contains not_contains present absent` — and two collection operators, `each` and `none`. `present` and `absent` take no `expected`. `not_matches` holds exactly when `matches` would not, on a scalar only. `matches` takes a Go `regexp` (RE2) pattern, matched unanchored and case-sensitively against the whole value of a scalar or against each element of a list, where `.` does not match a newline; lint compiles every pattern at load. Comparison is typed by the registry, so `"0"` and `0` cannot be confused; `lt`…`gte` require a numeric type.
...
`params` declares thresholds with types and defaults; judgments reference them (section 6.3). In stage 1 only the defaults exist. A default is the guide's criterion even where a distribution ships a different value; the control text names the distribution default and the parameter that relaxes it. `--tuning <file>` (organisation values) and profiles (`{extends, include, exclude, params, severity}`, with a built-in profile named `default`) arrive in stage 3. The parameter values in force are recorded in the result (D18).
...
| symlink, FIFO, device, oversized or binary file | refused or truncated by the read primitive; `error` / `truncated`; never blocks (a file binary by design goes through `ReadFileBinary`, same guard, same cap) |
...
- **Summary.** A summary block always precedes the table, in three parts: automatic verdicts by severity (`PASS`, `FAIL`, `WARN`), items under manual review (`MANUAL` and partial-control `WARN`), and undecidable items (`ERROR`, `NOT_APPLICABLE`, `WAIVED`) — followed by the count of facts that failed to collect and the waivers expiring within 30 days. Since stage 3B the summary also carries `scopes`: the same three parts once for the guide (every control with a KISA reference) and once for beyond it (category `beyond`), which sum to the totals; the table prints one line per scope and a separator before the first row beyond the guide, and rows sort by scope before severity (D29). There is no single hardening score; a ratio, if shown, states its denominator and excludes `MANUAL` and `NOT_APPLICABLE` (D18).
...
Against the 2026 list this gives **50 auto, 8 partial, 9 deferred, 0 manual-only, 0 not-applicable** — 58 of 67 judged automatically in some form. The per-item table is appendix A; from stage 2 it is regenerated by joining the control set (ids, `automation`) with the reference inventory in `docs/reference/kisa/` (item names), and CI checks that the committed table matches.
...
**Stage 2 — both distributions, every automatable item.** Collectors complete: sshd (`-G` → `-T` → parse fallback with method recorded; Match personas; include sources), services (socket activation, masked/static/indirect, logical names), PAM (authselect / pam-auth-update / manual detection, stack expansion, derived pwquality and faillock with sources), firewall (backend detection, raw dumps, minimal normalised model with confidence), logging as a function (journald-only hosts), network sysctls with the kernel's per-parameter composition of `all` and per-interface values (maximum for `rp_filter`, logical OR for `send_redirects`, `accept_redirects` depending on that interface's forwarding) with `default` collected as the template for future interfaces and never folded into an effective value, and IPv6 pairs; the whole `/proc/sys` tree; MAC status (SELinux/AppArmor, runtime versus config); NSS remote-source detection; inetd/xinetd; banners; time synchronisation; `snmpd.conf` (versions enabled, communities redacted to default/length/source-restriction); patch hygiene from cached metadata; account state (hash algorithm, empty passwords); the `env` block; ACL entries. All 58 auto and partial items enrolled with fixtures; the 9 deferred items enrolled as manual with evidence. Every control gains `references.stig` and `references.nist_800_53` where a mapping exists (section 3). Stage 2 runs as twelve plans — 2A foundations (permission-fact template, drop-in merge helper, coverage-table generator, CI matrix skeleton, reference index generator, the vocabulary additions), 2B accounts, 2C PAM, 2D sshd and banners, 2E home directories and the shell environment, 2G services and super-servers, 2F system files, startup and cron, 2H firewall, 2I logging and time synchronisation, 2J NFS, SNMP and patch hygiene, 2L FTP/mail/DNS evidence with the deferred enrolments, 2M the coverage and reference gate; the walk-dependent U-15, U-23, U-25 and U-33 waited for stage 3A. Rocky and AlmaLinux are verified in CI's init containers during stage 2; a run on a real VM precedes the public release. CI matrix (`ubuntu:22.04`, `ubuntu:24.04`, `rockylinux/rockylinux:9-ubi-init`, `almalinux/9-init`, `debian:12` as a canary), `sudo muster collect` on the GitHub runner VM, the capability-matrix test, the non-root job. Coverage table generated and committed. Parser oracle tests. Example snapshots captured from public images. `snapshot extract`, `controls new`, `CONTRIBUTING.md`.

**Stage 3 — high-value checks beyond the list, from the same collector.** 3A (merged): the walk itself, joined with package ownership and declared modes (rpm's file table; dpkg's lists, `statoverride` and a per-release reference list) — `2026-09-16-stage3a-walk-design.md`. 3B (merged): the first nineteen controls beyond the guide — kernel self-protection sysctls and the three-source core-dump policy, the boot chain (grub.cfg permissions and password, Secure Boot state), separate partitions and mount options, swap encryption, module blacklists with built-in detection — from six read-only collectors, and the report's two scopes — `2026-09-18-stage3b-kernel-boot-mount-design.md`. Then: package verification (`rpm -V` / `dpkg --verify`, noise filtered and the filter recorded; 3C); file capabilities and ACLs in the walk; audit pipeline health (auditd rules present and immutable, journald persistent, remote forwarding, sudo logging, a file-integrity tool installed and scheduled); exposure cross-check (socket → process → package versus firewall, only when firewall confidence is full); root-equivalent paths (container-runtime sockets and their groups, `ld.so.preload`, writable `ExecStart` of root units, root's `PATH`); dormant accounts; `sudoers` `NOPASSWD`/`ALL` beyond U-63's permission check; processes running deleted executables; `authorized_keys` inventory; `fix --dry-run` with risk ordering, backups, validation commands and rollback; profiles realised (section 6.6): `kisa-unix-2026` stays the default and a `cis-<distro>-l1` profile selects controls and parameters for the CIS Level 1 server recommendations of the target distributions from the same collector, with `references.cis` as those controls' primary reference; tuning; `--anonymize` with its invariant test; `--max-age`; control YAML mutation testing; parser fuzzing.
...
Per-service configuration items (mail, DNS, FTP daemon configuration — the 9 deferred items), multi-host aggregation, an agentless SSH mode, file-integrity baselines, certificate expiry, OS end-of-life detection, cloud VM items, an ISMS-P mapping (the `references` key is reserved), configuration files, kernel lockdown and command-line checks, Ansible output, APT/YUM repositories, Homebrew, Docker images, a check-only build for macOS and Windows.

### 10.4 Never

CVE matching, runtime detection, container and Kubernetes benchmarks, external port scanning, applying remediation, a single hardening score, CIS Benchmark text (D24), and other asset classes in this codebase (Windows, DBMS, web/WAS, network and security devices: sibling tools sharing the contracts).
...
**Stage 3 (3F, `2026-09-17-stage3f-hardening-design.md`).** Mutation testing of control YAML (operator flip, expected-value substitution, clause removal, distribution-condition removal; 100 % kill rate with a documented exclusion list; gremlins on custom functions only). Native fuzzing of every parser with committed seed corpora: the seed corpus is the pull-request regression (it runs inside `go test`), the fuzzing itself runs nightly, sixty seconds per target over four shards (3F amends the earlier "thirty seconds per parser on pull requests"). The daemon oracles of stage 2 and the example snapshots land in 3F as well.
...
- **D29 — Checks beyond the guide run by default; the summary says which scope a verdict belongs to; the exit code does not change.** Stage 3B adds controls that are not KISA items (`category: beyond`). They run in every `check` and the summary gains `scopes` — the guide and beyond it, each with the three parts of section 9 — while every existing summary field keeps its meaning and rows sort by scope first; a FAIL beyond the guide is a FAIL for the exit code, and the profile of 3D, not a flag, is what gives a KISA-only user the choice. Three departures ride on this decision: a sysctl's verdict reads its running value (`default_on: effective`, not the `both` of section 5.3, because a kernel-compiled default has no persisted line and would report a drift that is not one); the observations of `files.user_rhosts`, `files.env_files` and `files.dev_nondevice` carry the subject kind `file:` instead of `item:`, which renames the waiver keys that name them; and a control whose judged facts a collector cannot produce on the supported hosts records the mutant as equivalent rather than shipping a fixture the host could never write.
...
| U-56 | FTP 서비스 접근 제어 설정 | 하 | partial (whether the user list allows or denies depends on a setting v1 does not parse) |
...
Totals: auto 50, partial 8, deferred 9.
