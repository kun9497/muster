...
**Global references.** `references.cis` (benchmark name, version, recommendation number) is present from stage 1. Stage 2 adds `references.stig` (the DISA STIG id and rule for the matching OS release) and `references.nist_800_53` (control ids) wherever a mapping exists; adding a key under `references` does not change the control schema version. References are numbers only — no CIS Benchmark or STIG text — and muster judges its own controls, never a benchmark's official compliance level (D24).
